...
static void lis2duxs12_config(const struct device *lis2duxs12)
{
	...
	/* set LIS2DUXS12 accel sampling frequency to 200 Hz */
	odr_attr.val1 = 200;
	odr_attr.val2 = 0;

	if (sensor_attr_set(lis2duxs12, SENSOR_CHAN_ACCEL_XYZ,
			    SENSOR_ATTR_SAMPLING_FREQUENCY, &odr_attr) < 0) {
		printk("Cannot set sampling frequency for LIS2DUXS12 accel\n");
		return;
	}
	...
	if (sensor_attr_set(lis2duxs12, SENSOR_CHAN_ACCEL_XYZ,
			    SENSOR_ATTR_FULL_SCALE, &fs_attr) < 0) {
		printk("Cannot set full scale for LIS2DUXS12 accel\n");
		return;
	}
